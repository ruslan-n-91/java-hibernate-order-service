...
CREATE SEQUENCE IF NOT EXISTS hibernate_orders_id_seq START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS orders
(
    id           BIGINT PRIMARY KEY DEFAULT nextval('hibernate_orders_id_seq'),
    order_number VARCHAR(255) NOT NULL,
    status       VARCHAR(255) NOT NULL,
    created_at   TIMESTAMP    NOT NULL
);

ALTER SEQUENCE hibernate_orders_id_seq OWNED BY orders.id;
